    ...
    wx.ongTouchMove = function(evt) {
      ...
      if (disPlayItem) {
        if (item) {
          ...
          let outNe = new GMLMouseEvent(GMLMouseEvent.Out, {
            globelX: evt.x,
            globelY: evt.y
          });
          ...
          let overNe = new GMLMouseEvent(GMLMouseEvent.Over, {
            globelX: evt.x,
            globelY: evt.y
          });
          ...
          BaseScene.main._defaultOverDisItem = disPlayItem;
          let ne2 = new GMLMouseEvent(GMLMouseEvent.Over, {
            globelX: evt.x,
            globelY: evt.y
          });
        ...
        if (item) {
          BaseScene.main._defaultOverDisItem = null;
          let ne = new GMLMouseEvent(GMLMouseEvent.Out, {
            globelX: evt.x,
            globelY: evt.y
          });
...
class BaseNotificationCenter extends BaseObject {
  static get main() {
    if (!wx.gmlNotificationCenter)
      wx.gmlNotificationCenter = new BaseNotificationCenter();
    return wx.gmlNotificationCenter;
  }
...
class GMLShape extends GMLDisplay {
  ...
  drawInContext(ctx, offsetX, offsetY, offsetScaleX, offsetScaleY) {
    ...
    let quilaty = 1.0 / wx.sysInfo.pixelRatio
    //开始绘制
    if (this._fColorStr != "#0") {
      //绘制背景
      ctx.setFileStyle(this._fColorStr);
    ...
    if (this._sColorStr != "#0") {
      //绘制边框
      ctx.setStrokeStyle(this._sColorStr);
...
class GMLImage extends GMLDisplay {
  ...
  drawInContext(ctx, offsetX, offsetY, offsetScaleX, offsetScaleY) {
    ...
    if (this.img) {
      ...
      this._rectVect[1] -= this._rectVect[3] * this._itiwY;
      let quilaty = 1.0 / wx.sysInfo.pixelRatio
      ctx.drawImage(this.imgPath, this.zhuaquRect[0], this.zhuaquRect[1], this.width, this.height, this._rectVect[0] * quilaty, this._rectVect[1] * quilaty, this._rectVect[2] * quilaty, this._rectVect[3] * quilaty);
      ctx.restore();
    }
...
class GMLStaticTextField extends GMLDisplay {
  ...
  drawInContext(ctx, offsetX, offsetY, offsetScaleX, offsetScaleY) {
    ...
    let quilaty = 1.0 / wx.sysInfo.pixelRatio
    //设置文本样式
    ctx.setTextAlign(this._hAliginment);
    ctx.setFontSize(this._fontSize * tOffsetScaleX * quilaty);//(this._fontSize * tOffsetScaleX * quilaty) + "px " + this._fontName + " " + (this.isBold ? "bold" : "solid");
    ctx.setFillStyle(this._fontColorStr);
...
class TimeLine extends BaseObject{
    ...
    set frameRate(n){
        ...
        this._frameRate = this._frameRate > 50 ? 50 : this._frameRate;//最大50帧
...
class ResourceManager extends BaseObject {
  static get main() {
    if (!wx.resourceManager)
      wx.resourceManager = new ResourceManager();
    return wx.resourceManager;
  }
  constructor() {
    super();
    ...
    if (typeof(callBackFunc) == "function" && observer && tempUrl != "") {
      ...
      if (this._imgMap.has(tempUrl)) {
        ...
        if (ResourceManager.main._waitLoadimgMap.has(tempUrl)) {
          ...
          ResourceManager.main._waitLoadimgMap.set(tempUrl, new Set([{
            "observer": observer,
            "callBackFunc": callBackFunc
          }]))
        }
...
module.exports = {
  ...
  "GMLSprite": GMLSprite,
  "GMLImage": GMLImage,
  "GMLStaticTextField": GMLStaticTextField,
  "BaseEvent": BaseEvent,
  "GMLMouseEvent": GMLMouseEvent,
  "GMLKeyBoardEvent": GMLKeyBoardEvent,
  "GMLEvent": GMLEvent,
  "GMLMedia": GMLMedia,
  "GMLAudio": GMLAudio,
  "GMLVideo": GMLVideo,
